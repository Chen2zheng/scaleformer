...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs, l2, l3, l4 = model(image_batch)
            # outputs, l2, l3 = model(image_batch)
            # outputs = F.interpolate(outputs, size=label_batch.shape[1:], mode='bilinear', align_corners=False)

            loss_ce = ce_loss(outputs, label_batch[:].long())
            loss_dice = dice_loss(outputs, label_batch[:], softmax=True)
            l1_loss = loss_dice * 0.7 + loss_ce * 0.3

            l2_ce = ce_loss(l2, label_batch[:].long())
            l2_dice = dice_loss(l2, label_batch, softmax=True)
            l2_loss = l2_ce * 0.3 + l2_dice * 0.7

            l3_ce = ce_loss(l3, label_batch[:].long())
            l3_dice = dice_loss(l3, label_batch, softmax=True)
            l3_loss = l3_ce * 0.3 + l3_dice * 0.7

            l4_ce = ce_loss(l4, label_batch[:].long())
            l4_dice = dice_loss(l4, label_batch, softmax=True)
            l4_loss = l4_ce * 0.3 + l4_dice * 0.7
            
            loss = 0.7 * l1_loss + 0.1 * l2_loss + 0.1 * l3_loss + 0.1 * l4_loss
            # loss = 0.6 * l1_loss + 0.2 * l2_loss + 0.2 * l3_loss 
        ...
        if (epoch_num + 1) >= 240 and (epoch_num + 1) % 5 == 0:
